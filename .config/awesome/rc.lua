...
local terminal = os.getenv("TERMCMD") or "kitty -c /home/kvl/.config/kitty/kitty-gruvbox.conf"
local notes    = os.getenv("NOTES") or "/home/kvl/Applications/Obsidian-0.15.9.AppImage"
local browser  = os.getenv("BROWSER") or "firefox"
local editor   = os.getenv("EDITOR") or "vscode"
local editornvim   = os.getenv("EDITORNVIM") or "nvim"
local editor_cmd = terminal .. " -e " .. editornvim .. " "
...
local mytextclock = wibox.widget.textclock(beautiful.widget_markup:format(beautiful.bg_normal, "🕓 %r"), 1)
-- mytextclock:timezone("UTC")
-- local mytextclock = wibox.widget.textclock('<span font="FiraCode-bold 12">🕓 %I:%M %p</span>')
...
local tags = { "󰎤 : 󰞷", "󰎧 : 󰨞", "󰎪 : 󰀶", "󰎭 : 󰈹", "󰎱 : 󱓥", "󰎳 : 󰏆", "󰎶 : 󰈦", "󰎹 : 󰚤", "󰎼 : 󰒓","󰽽 : 󰣛" }
-- local tags = { "❶", "❷", "❸", "❹", "❺", "❻" }
...
-------------------------------------------------------------------------------
  -- Create the wibar
  s.mywibox = awful.wibar({position = "top", screen = s, height = beautiful.wibar_height, border_width = "16"})
  -- s.mywibox = awful.wibar({position = "top", screen = s, height = beautiful.wibar_height, border_width = "12"})
...
modes.launcher = gears.table.join(
  {
    ...
    },
    {
      ...
      pattern = {'f'},
      handler = function() awful.spawn(terminal.." -e lfub") end
    },
    {
      description = "launch Pcmanfm",
      pattern = {'F'},
      handler = function() awful.spawn.raise_or_spawn("pcmanfm") end
    },
    -- {
    ...
    -- },
    {
      ...
      pattern = {'k'},
      handler = function() awful.spawn.raise_or_spawn("kdeconnect-app") end
    },
    {
      description = "launch vieb",
      pattern = {'v'},
      handler = function() awful.spawn.raise_or_spawn("vieb") end
    },
    {
      description = "launch Pavucontrol",
      pattern = {'p'},
      handler = function() awful.spawn.raise_or_spawn("pavucontrol") end
    },
    {
      description = "launch SS",
      pattern = {'c'},
      handler = function() awful.spawn.raise_or_spawn("flameshot gui") end
    },
    {
      description = "launch VScode",
      pattern = {'e'},
      handler = function() awful.spawn.raise_or_spawn("vscode") end
    },
    {
      description = "launch Notes",
      pattern = {'o'},
      handler = function() awful.spawn(notes) end
    },
    {
    ...
    },
    {
      ...
      pattern = {'l'},
      handler = function() awful.spawn.raise_or_spawn("i3lock") end
    },
    -- {
      ...
      handler = function(mode)
        run_shell.launch{
          ...
          exe_callback = function(command)
            local search = " https://duckduckgo.com/?q=!g+" .. command:gsub('%s', '+')
...
require("awful").screen.focused().padding = { top = "0", bottom = "16", left = "16", right = "16" }
awful.rules.rules = {
  ...
  { rule_any = { class = { "Plank" }},
    ...
    properties = { ontop = true, floating = true, focus = custom_focus_filter }},
  ...
  { rule_any = { class = { "org.pwmt.zathura" }},
    except_any = { name = { "Unlock Database - KeePassXC", "Auto-Type - KeePassXC" }},
...
--
awful.util.spawn_with_shell("/home/kvl/.local/bin/autostart.sh")
